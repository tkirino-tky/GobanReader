...
class SgfWriter(private val context: Context) {
    ...
    /**
     * SGFファイルを Documents/GobanReader フォルダに自動命名で保存する
     * UIをブロックしないよう非同期（suspend）で処理する
     */
    suspend fun saveSgfFileAutoNamed(sgfContent: String): Result<File> {
        return withContext(Dispatchers.IO) {
            runCatching {
                // 1. 保存先フォルダの決定 (ユーザーが見える Documents/GobanReader)
                val documentsDir = Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOCUMENTS)
                val appDir = File(documentsDir, "GobanReader")
